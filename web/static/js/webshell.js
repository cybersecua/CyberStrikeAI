...
function getWebshellConnections() {
    ...
    return apiFetch('/api/webshell/connections', { method: 'GET' })
        ...
        .catch(function (e) {
            console.warn('read WebShell Connection ListFailed', e);
            return [];
        });
}

// fromserviceRefreshConnection List
...
// use wsT avoidandglobal window.t noRecursion
function wsT(key) {
    ...
    var fallback = {
        ...
        'webshell.cmdParamPlaceholder': 'Default is cmd, e.g. if set to xxx the request becomes xxx=command',
        'webshell.connections': 'Connection List',
        'webshell.noConnections': 'No connections. Click "Add Connection"',
        'webshell.selectOrAdd': 'Select from left, or add new WebShell connection',
 ...
 'webshell.dbNoColumns': 'nocolumnInfo',
        ...
        'webshell.dbRows': 'line',
        'webshell.dbColumns': 'column',
 ...
 'webshell.terminalWelcome': 'WebShell Virtual Terminal - inputcommandafterbyenter keyline(Ctrl+L clear screen)',
        ...
        'webshell.listDir': 'columnoutdirectory',
 ...
 'webshell.testNoExpectedOutput': 'Shell returnresponsebutnot yettooutput,pleasecheckconnectionPasswordandCommand Parameter Name',
        ...
        'webshell.terminalRunning': 'executing',
        'webshell.terminalCopyOk': 'LogCopied',
 ...
 'webshell.newFile': 'newfile',
        ...
        'webshell.breadcrumbHome': 'based on',
        'webshell.dirTree': 'directory list',
        'webshell.searchPlaceholder': 'searchconnection...',
 'webshell.noMatchConnections': 'nomatchconnection',
        'webshell.batchProbe': 'one-click batch probe',
        'webshell.probeRunning': 'probing',
 'webshell.probeOnline': 'at',
        'webshell.probeOffline': 'Offline',
 'webshell.probeNoConnections': 'nocanconnection',
        'webshell.back': 'return',
        'webshell.colModifiedAt': 'Modified',
        'webshell.colPerms': 'permissions',
        'webshell.colOwner': 'owner',
 'webshell.colGroup': 'use',
...
function renderWebshellList() {
 ...
 listEl.innerHTML = '<div class="webshell-empty">' + (wsT('webshell.noMatchConnections') || 'nomatchconnection') + '</div>';
        return;
    }
 ...
 probeHtml = '<span class="webshell-probe-badge ok">' + (wsT('webshell.probeOnline') || 'at') + '</span>';
...
function updateWebshellBatchProbeButton(done, total, okCount) {
    ...
    if (typeof done === 'number' && typeof total === 'number' && total > 0 && typeof okCount === 'number') {
        btn.textContent = (wsT('webshell.batchProbe') || 'one-click batch probe') + ' (' + okCount + '/' + total + ')';
    } else {
        btn.textContent = wsT('webshell.batchProbe') || 'one-click batch probe';
    }
}
...
function runBatchProbeWebshellConnections() {
 ...
 alert(wsT('webshell.probeNoConnections') || 'nocanconnection');
        return;
    }
...
function setWebshellTerminalStatus(running) {
    ...
    el.textContent = running ? (wsT('webshell.terminalRunning') || 'executing') : (wsT('webshell.terminalIdle') || 'idle');
}
...
function renderWebshellTerminalSessions(conn) {
    ...
    html += '<button type="button" class="webshell-terminal-session-add" data-action="add" title="' + escapeHtml(wsT('webshell.terminalNewWindow') || 'newTerminal') + '">+</button>';
    ...
    tabsEl.querySelectorAll('[data-action]').forEach(function (btn) {
        btn.addEventListener('click', function () {
            ...
            if (action === 'close') {
                ...
                // cleanupthisTerminal's Logandhistory
...
function renderWebshellAiErrorMessage(targetEl, rawMessage) {
    ...
    if (full && full !== shortMsg) {
        ...
        summary.textContent = 'viewdetailederror';
...
function buildWebshellTimelineItemFromDetail(detail) {
    ...
    if (eventType === 'iteration') {
        title = ap + ((typeof window.t === 'function') ? window.t('chat.iterationRound', { n: data.iteration || 1 }) : (' ' + (data.iteration || 1) + ' iteration'));
    ...
    if (eventType === 'tool_call' && data && (data.argumentsObj || data.arguments)) {
        try {
            ...
            if (args && typeof args === 'object') {
                var paramsLabel = (typeof window.t === 'function') ? window.t('timeline.params') : 'parameter:';
        ...
        var noResultText = (typeof window.t === 'function') ? window.t('timeline.noResult') : 'no results';
 ...
 var execIdLabel = (typeof window.t === 'function') ? window.t('timeline.executionId') : 'lineID:';
...
function selectWebshell(id, stateReady) {
    ...
    workspace.innerHTML =
        ...
        '<span class="webshell-quick-label">' + (wsT('webshell.quickCommands') || 'quick command') + ':</span> ' +
        ...
        '<div class="webshell-row-actions-menu">' +
        '<button type="button" class="btn-ghost" id="webshell-mkdir-btn">' + (wsT('webshell.newDir') || 'create directory') + '</button>' +
 '<button type="button" class="btn-ghost" id="webshell-newfile-btn">' + (wsT('webshell.newFile') || 'newfile') + '</button>' +
 '<button type="button" class="btn-ghost" id="webshell-upload-btn">' + (wsT('webshell.upload') || 'on') + '</button>' +
        ...
        '<button type="button" class="btn-primary btn-sm webshell-ai-new-btn" id="webshell-ai-new-conv">' + (wsT('webshell.aiNewConversation') || 'newconversation') + '</button>' +
        ...
        terminalCopyLogBtn.addEventListener('click', function () {
            ...
            if (navigator && navigator.clipboard && navigator.clipboard.writeText) {
                navigator.clipboard.writeText(log).then(function () {
                    terminalCopyLogBtn.title = wsT('webshell.terminalCopyOk') || 'LogCopied';
                    setTimeout(function () {
 ...
 if (!confirm(wsT('webshell.dbDeleteProfileConfirm') || 'OKDeletethisdataconnectionconfiguration?')) return;
 ...
 html += '<div class="webshell-db-column-empty">' + escapeHtml(wsT('webshell.dbNoColumns') || 'nocolumnInfo') + '</div>';
                }
                html += '</details>';
            });
            html += '</div></details>';
    ...
    function loadDbSchema() {
        if (!conn || !conn.id) {
            webshellDbSetOutput(wsT('webshell.dbNoConn') || 'Please select WebShell connection', true);
            return;
        }
        if (webshellRunning) {
            webshellDbSetOutput(wsT('webshell.dbRunning') || 'Database command executing, please wait', true);
            return;
        }
        var cfg = webshellDbCollectConfig(conn);
        resetDbColumnLoadCache();
    ...
    function runDbQuery(isTestOnly) {
        ...
        execWebshellCommand(conn, built.command).then(function (out) {
            ...
            if (isTestOnly) {
                if (success) {
                    cfg.output = 'connectionTestvia';
                    cfg.outputIsError = false;
                    saveWebshellDbConfig(conn, cfg);
                    webshellDbSetOutput(cfg.output, false);
                } else {
                    cfg.output = 'connectionTestFailed' + (content ? (':\n' + content) : '');
            ...
            if (hasTable) {
                cfg.output = wsT('webshell.dbExecSuccess') || 'SQL execution succeeded';
                cfg.outputIsError = false;
                saveWebshellDbConfig(conn, cfg);
                webshellDbSetOutput(cfg.output, false);
            } else {
...
function loadWebshellAiHistory(conn, messagesContainer) {
    ...
    return apiFetch('/api/webshell/connections/' + encodeURIComponent(conn.id) + '/ai-history', { method: 'GET' })
        ...
        .catch(function (e) {
            console.warn('load WebShell AI historyFailed', conn.id, e);
        });
}
...
function runWebshellAiSend(conn, inputEl, sendBtn, messagesContainer) {
 ...
 // first conversationId out,avoidafterasynccallbackin eventData byaftereventoverride undefined throw error
                        ...
                        var iterTitle = (typeof window.t === 'function')
                            ...
                            : (iterN ? (' ' + iterN + ' iteration') : (eventData.message || 'Iteration'));
                        ...
                        var titleText = (typeof window.t === 'function')
                            ...
                            : (tname + (success ? ' execution complete' : ' execution failed'));
        ...
        renderWebshellAiErrorMessage(assistantDiv, 'requestAbnormal: ' + (err && err.message ? err.message : String(err)));
    }).then(function () {
...
// linequick commandwilloutputwritecurrentTerminal
...
function initWebshellTerminal(conn) {
 ...
 appendWebshellTerminalLog(terminalKey, '^C (currentVersionnotsupportincommand)\n');
            }
            webshellLineBuffer = '';
            webshellHistoryIndex = -1;
            term.write(WEBSHELL_PROMPT);
        ...
        if (data === '\r' || data === '\n') {
            ...
            if (cmd) {
                if (webshellRunning) {
                    ...
                    appendWebshellTerminalLog(terminalKey, (wsT('webshell.waitFinish') || 'pleasewaitcurrentcommandexecution complete') + '\n');
                    term.write(WEBSHELL_PROMPT);
                    return;
                }
                pushWebshellHistory(terminalKey, cmd);
...
// callbackendlinecommand
function execWebshellCommand(conn, command) {
    return new Promise(function (resolve, reject) {
        if (typeof apiFetch === 'undefined') {
            reject(new Error('apiFetch not defined'));
            return;
        }
...
function webshellFileListDir(conn, path) {
    ...
    if (typeof apiFetch === 'undefined') {
        listEl.innerHTML = '<div class="webshell-file-error">apiFetch not defined</div>';
        return;
    }
...
function renderFileList(listEl, currentPath, rawOutput, conn, nameFilter) {
 ...
 // 
    ...
    if (breadcrumbEl) {
        ...
        breadcrumbEl.innerHTML = '<a href="#" class="webshell-breadcrumb-item" data-path=".">' + (wsT('webshell.breadcrumbHome') || 'based on') + '</a>' +
...
function renderDirectoryTree(currentPath, items, conn) {
    ...
    if (!state) {
 treeEl.innerHTML = '<div class="webshell-empty">nodirectory</div>';
        return;
    }
    var tree = state.tree;
 ...
 // ensurecurrentpathfirstexistexpand
...
function webshellFileMkdir(conn, pathInput) {
    ...
    var name = prompt(wsT('webshell.newDir') || 'create directory', 'newdir');
...
function webshellFileUpload(conn, pathInput) {
    ...
    input.onchange = function () {
        ...
        reader.onload = function () {
            ...
            var listEl = document.getElementById('webshell-file-list');
 if (listEl) listEl.innerHTML = '<div class="webshell-loading">' + (wsT('webshell.upload') || 'on') + '...</div>';
            var idx = 0;
            function sendNext() {
...
function refreshWebshellUIOnLanguageChange() {
    ...
    if (workspace) {
        if (!currentWebshellId || !webshellCurrentConn) {
            ...
            if (quickLabel) quickLabel.textContent = (wsT('webshell.quickCommands') || 'quick command') + ':';
 ...
 // File Managertoolbutton(area):switchlanguagewhenimmediatelyupdate
            ...
            if (mkdirBtn) mkdirBtn.textContent = wsT('webshell.newDir') || 'create directory';
 if (newFileBtn) newFileBtn.textContent = wsT('webshell.newFile') || 'newfile';
 if (uploadBtn) uploadBtn.textContent = wsT('webshell.upload') || 'on';
            ...
            var dbTemplateBtn = document.getElementById('webshell-db-template-btn');
            if (dbTemplateBtn) dbTemplateBtn.textContent = wsT('webshell.dbTemplateSql') || 'Example SQL';
            var dbClearBtn = document.getElementById('webshell-db-clear-btn');
            if (dbClearBtn) dbClearBtn.textContent = wsT('webshell.dbClearSql') || 'clear SQL';
            var dbRunBtn = document.getElementById('webshell-db-run-btn');
 if (dbRunBtn) dbRunBtn.textContent = wsT('webshell.dbRunSql') || 'line SQL';
            var dbTestBtn = document.getElementById('webshell-db-test-btn');
...
function testWebshellConnection() {
    ...
    if (!url) {
 alert(wsT('webshell.url') ? (wsT('webshell.url') + ' Required') : 'please Shell address');
        return;
    }
