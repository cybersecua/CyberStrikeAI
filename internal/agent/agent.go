...
func (a *Agent) formatMinimalNotification(executionID string, toolName string, size int, lineCount int, filePath string) string {
	...
	sb.WriteString(fmt.Sprintf("Tool output was too large to inline (execution ID: %s). It has been stored and is queryable.\n\n", executionID))
	...
	sb.WriteString("\n")
	sb.WriteString("Query the stored result via query_execution_result:\n")
	sb.WriteString(fmt.Sprintf(" - Paginated read:        query_execution_result(execution_id=\"%s\", page=1, limit=100)\n", executionID))
	sb.WriteString(fmt.Sprintf(" - Plain-text search:     query_execution_result(execution_id=\"%s\", search=\"<term>\")\n", executionID))
	sb.WriteString(fmt.Sprintf(" - Filter by substring:   query_execution_result(execution_id=\"%s\", filter=\"error\")\n", executionID))
	sb.WriteString(fmt.Sprintf(" - Regex match:           query_execution_result(execution_id=\"%s\", search=\"\\\\d+\\\\.\\\\d+\\\\.\\\\d+\\\\.\\\\d+\", use_regex=true)\n", executionID))
	sb.WriteString("\n")
	if filePath != "" {
		sb.WriteString("If query_execution_result is not available, the raw output is on disk at the File path above. Use these shell alternatives:\n")
		sb.WriteString("\n")
		sb.WriteString("**Paginate by line range:**\n")
		sb.WriteString(fmt.Sprintf(" - First 100 lines:       exec(command=\"head\", args=[\"-n\", \"100\", \"%s\"])\n", filePath))
		sb.WriteString(fmt.Sprintf(" - Last 100 lines:        exec(command=\"tail\", args=[\"-n\", \"100\", \"%s\"])\n", filePath))
		sb.WriteString(fmt.Sprintf(" - Lines 50-150:          exec(command=\"sed\", args=[\"-n\", \"50,150p\", \"%s\"])\n", filePath))
		sb.WriteString("\n")
		sb.WriteString("**Match by pattern:**\n")
		sb.WriteString(fmt.Sprintf(" - Fixed substring:       exec(command=\"grep\", args=[\"<term>\", \"%s\"])\n", filePath))
		sb.WriteString(fmt.Sprintf(" - Regex (e.g. IP):       exec(command=\"grep\", args=[\"-E\", \"\\\\d+\\\\.\\\\d+\\\\.\\\\d+\\\\.\\\\d+\", \"%s\"])\n", filePath))
		sb.WriteString(fmt.Sprintf(" - Case-insensitive:      exec(command=\"grep\", args=[\"-i\", \"<term>\", \"%s\"])\n", filePath))
		sb.WriteString(fmt.Sprintf(" - Show line numbers:     exec(command=\"grep\", args=[\"-n\", \"<term>\", \"%s\"])\n", filePath))
		sb.WriteString("\n")
		sb.WriteString("**Summarize:**\n")
		sb.WriteString(fmt.Sprintf(" - Line count only:       exec(command=\"wc\", args=[\"-l\", \"%s\"])\n", filePath))
		sb.WriteString(fmt.Sprintf(" - Errors only:           exec(command=\"grep\", args=[\"error\", \"%s\"])\n", filePath))
		sb.WriteString(fmt.Sprintf(" - Drop blank lines:      exec(command=\"grep\", args=[\"-v\", \"^$\", \"%s\"])\n", filePath))
		sb.WriteString("\n")
		sb.WriteString("**Dump the whole file (use sparingly — the result is large for a reason):**\n")
		sb.WriteString(fmt.Sprintf(" - Built-in cat:          cat(file=\"%s\")\n", filePath))
		sb.WriteString(fmt.Sprintf(" - Via exec:              exec(command=\"cat\", args=[\"%s\"])\n", filePath))
		sb.WriteString("\n")
		sb.WriteString("**Rules of thumb:**\n")
		sb.WriteString(" - Prefer query_execution_result over the shell alternatives when available — it streams and paginates without re-spawning tools.\n")
		sb.WriteString(" - Narrow with a filter or line range before widening; do not cat a 5 MB file and read it all.\n")
		sb.WriteString(" - Use POSIX-compatible flags; avoid GNU-only extensions so the same recipes work on macOS / BSD targets.\n")
	}
